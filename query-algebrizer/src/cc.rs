...
impl ConjoiningClauses {
    ...
    pub fn constrain_column_to_entity(&mut self, table: TableAlias, column: DatomsColumn, entity: i64) {
        self.wheres.push(ColumnConstraint::EqualsEntity(QualifiedAlias(table, column), entity))
    }

    pub fn constrain_attribute(&mut self, table: TableAlias, attribute: i64) {
    ...
    /// Returns false if it's impossible for this type to apply.
    /// Constraints the var if there's no existing type.
    ...
    /// present.
    fn mark_as_tx(&mut self, tx: &PatternNonValuePlace) {
        match *tx {
    ...
    fn mark_as_ref(&mut self, value: &PatternNonValuePlace) {
    ...
    fn entid_for_ident<'s, 'a>(&self, schema: &'s Schema, ident: &'a NamespacedKeyword) -> Option<i64> {
        schema.get_entid(&ident.to_string())      // TODO: #291.
    }

    fn attribute_for_ident<'s, 'a>(&self, schema: &'s Schema, ident: &'a NamespacedKeyword) -> Option<&'s Attribute> {
        self.entid_for_ident(schema, ident).and_then(|x| schema.attribute_for_entid(x))
    }
    ...
    fn table_for_places<'s, 'a>(&self, schema: &'s Schema, attribute: &'a PatternNonValuePlace, value: &'a PatternValuePlace) -> Option<DatomsTable> {
        match attribute {
            ...
            &PatternNonValuePlace::Ident(ref kw) =>
                self.attribute_for_ident(schema, kw)
                    .and_then(|attribute| self.table_for_attribute_and_value(attribute, value)),
        }
    }
...
impl ConjoiningClauses {
    ...
    /// columns into `wheres`.
    /// For example, a bindings map like
    ///
    /// ```clojure
    ///   {?foo [:datoms12.e :datoms13.v :datoms14.e]}
    /// ```
    ///
    /// produces additional constraints:
    ///
    /// ```clojure
...
impl ConjoiningClauses {
    ...
    fn apply_pattern_clause_for_alias<'s>(&mut self, schema: &'s Schema, pattern: &Pattern, alias: &SourceAlias) {
        ...
        self.mark_as_tx(&pattern.tx);
        self.mark_as_ref(&pattern.entity);
        self.mark_as_ref(&pattern.attribute);
        ...
        match pattern.entity {
            ...
            PatternNonValuePlace::Variable(ref v) =>
                self.bind_column_to_var(alias.1.clone(), DatomsColumn::Entity, v.clone()),
            PatternNonValuePlace::Entid(entid) =>
                self.constrain_column_to_entity(alias.1.clone(), DatomsColumn::Entity, entid),
        ...
        match pattern.attribute {
            PatternNonValuePlace::Placeholder =>
                (),
            PatternNonValuePlace::Variable(ref v) =>
                self.bind_column_to_var(alias.1.clone(), DatomsColumn::Attribute, v.clone()),
            PatternNonValuePlace::Entid(entid) => {
                ...
                self.constrain_column_to_entity(alias.1.clone(), DatomsColumn::Attribute, entid)
            },
            PatternNonValuePlace::Ident(ref ident) => {
                if let Some(entid) = self.entid_for_ident(schema, ident) {
                    self.constrain_column_to_entity(alias.1.clone(), DatomsColumn::Attribute, entid);
        ...
        let attribute = self.get_attribute(schema, pattern);
        let value_type = attribute.map(|x| x.value_type);
        ...
        match pattern.value {
            ...
            PatternValuePlace::Variable(ref v) => {
                ...
                self.bind_column_to_var(alias.1.clone(), DatomsColumn::Value, v.clone());
            },
            PatternValuePlace::EntidOrInteger(i) =>
                ...
                if let Some(ValueType::Ref) = value_type {
                    self.constrain_column_to_entity(alias.1.clone(), DatomsColumn::Value, i);
                } else {
                    unimplemented!();
                },
            PatternValuePlace::IdentOrKeyword(ref kw) => {
                ...
                if let Some(ValueType::Ref) = value_type {
                    if let Some(entid) = self.entid_for_ident(schema, kw) {
                        self.constrain_column_to_entity(alias.1.clone(), DatomsColumn::Value, entid)
                    } else {
            ...
            PatternValuePlace::Constant(ref c) => {
                ...
                self.constrain_column_to_constant(alias.1.clone(), DatomsColumn::Value, c.clone().into_typed_value());
            },
        }
...
mod testing {
    ...
    fn test_apply_simple_pattern() {
        ...
        let d0_e = QualifiedAlias("datoms0".to_string(), DatomsColumn::Entity);
        let d0_a = QualifiedAlias("datoms0".to_string(), DatomsColumn::Attribute);
        let d0_v = QualifiedAlias("datoms0".to_string(), DatomsColumn::Value);

        // After this, we know a lot of things:
        assert!(!cc.is_known_empty);
        assert_eq!(cc.from, vec![SourceAlias(DatomsTable::Datoms, "datoms0".to_string())]);
    ...
    fn test_apply_two_patterns() {
        ...
        let d0_a = QualifiedAlias("datoms0".to_string(), DatomsColumn::Attribute);
        let d0_v = QualifiedAlias("datoms0".to_string(), DatomsColumn::Value);
        let d1_e = QualifiedAlias("datoms1".to_string(), DatomsColumn::Entity);
        let d1_a = QualifiedAlias("datoms1".to_string(), DatomsColumn::Attribute);

        assert!(!cc.is_known_empty);
        assert_eq!(cc.from, vec![
                   SourceAlias(DatomsTable::Datoms, "datoms0".to_string()),
                   SourceAlias(DatomsTable::Datoms, "datoms1".to_string()),
        ]);
