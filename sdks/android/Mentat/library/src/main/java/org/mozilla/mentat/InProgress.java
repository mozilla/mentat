...
public class InProgress extends RustObject {
    ...
    public TxReport transact(String transaction) {
        RustResult result = JNA.INSTANCE.in_progress_transact(this.validPointer(), transaction);
        if (result.isFailure()) {
            Log.e("InProgress", result.getError());
            return null;
        }
     ...
     */
    public void commit() {
        RustResult result = JNA.INSTANCE.in_progress_commit(this.consumePointer());
        result.logIfFailure("InProgress");
    }
    ...
    public void rollback() {
        RustResult result = JNA.INSTANCE.in_progress_rollback(this.consumePointer());
        result.logIfFailure("InProgress");
    }
